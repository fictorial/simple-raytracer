...
def make_world(image_size):
  ...
  for x in range(num_spheres):
    ...
    s=Sphere(c=vec3(
             ...
             -room_size+r,
             randrange(int(-room_size*8+r), int(room_size-r))), 
             r=r)
    s.material=Material("Sphere%d" % x)
    s.material.diffuse=RGB(0.5+random()-0.5, 0.5+random()-0.5, 0.5+random()-0.5)
    s.material.Ks=0.1
    w.materials.append(s.material)
    w.primitives.append(s)
  
  wall_material=Material(name="Wall", diffuse=RGB(0.5,0.5,0.5), Ks=0.3)
  w.materials.append(wall_material)
  
  floor_material=Material(name="Floor", diffuse=RGB(0.2,0.2,0.2), Ks=0.3)
  w.materials.append(floor_material)

  ceil_material=Material(name="Ceiling", diffuse=RGB(0.0, 0.4, 0.6), Ks=0.3)
  ...
  #back_plane=Plane(n=vec3(0,0,1), d=room_size*50)
  ...
  c=Camera(image_size)
  c.lookat(eye=vec3(-room_size/2.0,-room_size/4.0,room_size+25), at=vec3(0,0,0))
  c.focal_length=25
  w.cameras.append(c)
  w.active_camera=c
  ...
  for z in range(num_lights):
    l = Light(pos=vec3(randrange(-room_size,room_size), room_size/2.0-5.0, -z*room_size*5), 
              color=RGB(1,1,0))
    w.lights.append(l)
   
  #l=Light(pos=vec3(room_size/2.0, room_size/2-5.0, -room_size/2.0+5.0), color=RGB(1,1,0))
  #w.lights.append(l)
